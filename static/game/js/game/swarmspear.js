...
SwarmSpear.prototype.update = function(delta_ms) {
    ...
    if(this.active_time < swarm_appear_time) {
        ...
        this.sprite.rotation =
            this.direction + Math.PI - Math.PI * Math.pow(
                (swarm_appear_time - this.active_time) / swarm_appear_time,
                2
            );
    }
    else if(this.active_time < this.total_time - swarm_disappear_time) {
        this.sprite.alpha = 1;
        distance = swarm_initial_distance - swarm_initial_distance * (this.active_time - swarm_appear_time)
            / (swarm_travel_time);
    ...
    if(this.active_time > this.total_time) {
        this.removed = true;
    }
